const cardsCotnainer = document.querySelector('.cards-cont');
let fav = JSON.parse(localStorage.getItem("favMovies"));
const deleteMovie = (event,id)=>{
    event.preventDefault();
    const movie = fav.find((movie) => movie.id === id);
    if(movie){ 
    fav = fav.filter((favMovie) => favMovie.id !== id);
    ...
    if (array && array.length > 0) {
      array.forEach(movie => {
        if (movie.poster_path && movie.vote_average > 0) {
          ...
          card.href = `./details.html?id=${movie.id}`;
      ...
      cardsCotnainer.innerHTML = " <h1> No favorite movies found, <br> Please add movies to your favorites list</h1>";
